document.addEventListener('DOMContentLoaded', function() {
    ...
    // Obter valores dos prêmios existentes do servidor (se existirem)
    const existingPrizes = getExistingPrizesFromOldInput();
    
    // Inicializar campos de premiação com os valores existentes
    const initialValue = parseInt(numbersToDraw.value) || 0;
    if (initialValue > 0) {
        for (let i = 1; i <= initialValue; i++) {
            createPrizeField(i, existingPrizes[i] || '');
        }
        updateSubmitButton();
    } else {
        showNoPrizesMessage();
...
/**
 * Obtém os valores dos prêmios existentes dos inputs old() do Laravel
 * @returns {Object} Objeto com os valores dos prêmios indexados por posição
 */
function getExistingPrizesFromOldInput() {
    const prizes = {};
    
    // Tentar obter valores de inputs hidden ou data attributes que o Laravel pode ter renderizado
    // Isso será populado pelo Blade no modo edição
    for (let i = 1; i <= 10; i++) {
        const oldInput = document.querySelector(`input[name="prize[${i}]_old"]`);
        if (oldInput && oldInput.value) {
            prizes[i] = oldInput.value;
        }
    }
...
function updateNumbersToDraw(change) {
    ...
    if (change > 0) {
        ...
        // Decremento: remover último campo (com confirmação se tiver valor)
        const lastField = document.getElementById(`prize_${currentValue}`);
        if (lastField && lastField.value.trim() !== '') {
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    icon: 'question',
                    title: 'Confirmar Remoção',
                    html: `O campo <strong>${currentValue}º Lugar</strong> possui conteúdo que será perdido.<br><br>Deseja continuar?`,
                    ...
                    if (result.isConfirmed) {
                        ...
                        numbersToDraw.value = currentValue;
                    }
                });
            } else {
                if (confirm(`O campo ${currentValue}º Lugar possui conteúdo que será perdido. Deseja continuar?`)) {
                    removePrizeField(currentValue);
                } else {
                    // Restaurar valor anterior
                    numbersToDraw.value = currentValue;
 ...
 */
function createPrizeField(position, value = '') {
    ...
    input.required = true;
    input.value = value; // Definir valor se fornecido
    ...
    // Focar no campo recém-criado apenas se não tiver valor inicial
    if (!value) {
...
function updateSubmitButton() {
    ...
    if (quantity > 0) {
        ...
        btnSubmitForm.title = 'Configure pelo menos 1 prêmio antes de salvar';
    }
}
